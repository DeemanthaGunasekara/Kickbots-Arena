...
struct StepperState{
  ...
  unsigned long stepDelay = MIN_SPEED;
...
void setup(){
  ...
  Serial.println("Predictive Goalkeeper Receiver Ready");
...
void moveMotor(StepperState &motor,int stepPin,int dirPin){
  ...
  motor.stepDelay = MIN_SPEED - (MIN_SPEED-MAX_SPEED)*speedFactor;

  if(micros() - motor.lastStepTime >= motor.stepDelay){

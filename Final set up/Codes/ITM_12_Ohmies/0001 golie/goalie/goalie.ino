...
// ===== TRAVEL LIMIT =====
#define MAX_POSITION 400   // ±2 cm from center
...
void loop() {
  ...
  // ===== STEP BOTH MOTORS WITH HARD LIMIT =====
  if (motorEnabled && (micros() - lastStepTime >= currentDelay)) {

    // HARD LIMIT PROTECTION
    if ((motorDirection == HIGH && currentPosition <= -MAX_POSITION) ||
        (motorDirection == LOW  && currentPosition >=  MAX_POSITION)) {
      motorEnabled = false;
    } 
    else {
